...
Боту нужен файл `games.json` со списком appid. Способы ниже — от простого к муторному. Ни один не требует пароля, ключа API и двухфакторки.

## Способ 0. Кнопка в браузере (проще всего)
...
Внутри оно делает ровно то же, что Способ 1, только без консоли и без переноса файла руками.
